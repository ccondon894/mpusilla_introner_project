...
def main():
    if len(sys.argv) != 6:
        print("Usage: python make_all_samples_loci_flank_beds.py <metadata_file> <sample_name> <flank_length> <left_bed> <right_bed>")
        sys.exit(1)
    ...
    # Ensure sufficient flanking space based on orientation
    # For forward: need start >= flank_length (left flank goes upstream)
    # For reverse: need start >= flank_length (right flank goes upstream)
    sample_df = sample_df[sample_df['start'] >= flank_length]

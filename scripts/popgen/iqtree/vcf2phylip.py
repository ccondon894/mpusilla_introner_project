...
def is_invariant_site(column):
    """
    Check if a site is invariant (all samples have the same nucleotide, ignoring missing data)
    """
    # Get unique nucleotides, excluding missing data markers
    unique_nucleotides = set(column.upper())
    # Remove missing data markers
    unique_nucleotides.discard('N')
    unique_nucleotides.discard('-')
    unique_nucleotides.discard('?')
    
    # Site is invariant if only one nucleotide remains (or none, if all missing)
    return len(unique_nucleotides) <= 1
...
def main():
    ...
    with opener(args.filename, "rt") as vcf:
        ...
        print("SNPs excluded for being invariant sites: {:d}".format(snp_invariant))
